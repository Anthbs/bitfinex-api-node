...
class WSv2 extends EventEmitter {
  ...
  _handleTickerMessage (msg, chanData) {
    ...
    if (this._manageCandles) {
      const err = this._updateManagedTickers(symbol, data)
      ...
      data = this.tickers[symbol]
    } else if (this._transform) {
      data = Tick.unserialize(data)
    }
  ...
  _propagateMessageToListeners (msg, transform = this._transform) {
    ...
    for (let i = 0; i < listenerGroups.length; i++) {
      WSv2._notifyListenerGroup(listenerGroups[i], msg, transform)
    }
  }
   ...
   */
  static _notifyListenerGroup (lGroup, msg, transform) {
    ...
    if (!lGroup[eventName]) return

    for (let j = 0; j < lGroup[eventName].length; j++) {
      const { filter, cb } = lGroup[eventName][j]
      const ModelClass = lGroup[eventName][j].modelClass

      if (filter)  {
        filterByData = msg.filterOverride ? msg.filterOverride : data

        if (!WSv2._payloadPassesFilter(filterByData, filter)) {
          continue
        }
      }
      ...
      if (!transform) {
        ...
        cb(data.map((entry) => {
          return new ModelClass(entry)
        }))
      } else {
        cb(new ModelClass(data))
      }
    }
  }
